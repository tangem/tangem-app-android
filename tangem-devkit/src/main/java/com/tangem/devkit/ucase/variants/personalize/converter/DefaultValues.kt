...
class ConfigValuesHolder : BaseTypedHolder<Id, Value>() {
    ...
    fun init(default: PersonalizationConfig) {
        ...
        register(CardNumber.BatchId, Value(default.cardData.batch))
        register(Common.Curve, Value(default.curveID, Helper.listOfCurves()))
        register(Common.Blockchain, Value(default.cardData.blockchain, Helper.listOfBlockchain()))
        ...
        register(Token.Symbol, Value(default.cardData.token_symbol))
        register(Token.ContractAddress, Value(default.cardData.token_contract_address))
        register(Token.Decimal, Value(default.cardData.token_decimal))
        ...
        register(SettingsMask.OneApdu, Value(default.useOneCommandAtTime))
