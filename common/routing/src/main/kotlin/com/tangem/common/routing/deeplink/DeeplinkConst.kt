package com.tangem.common.routing.deeplink

object DeeplinkConst {
    const val DEEPLINK_KEY = "deeplink"
    const val WEBLINK_KEY = "link"

    const val TANGEM_SCHEME = "tangem"
    const val WALLET_ID_KEY = "user_wallet_id"
    const val CUSTOMER_WALLET_ID_KEY = "customer_wallet_id"
    const val CUSTOMER_ID_KEY = "customer_id"
    const val NETWORK_ID_KEY = "network_id"
    const val TYPE_KEY = "type"
    const val TOKEN_ID_KEY = "token_id"
    const val DERIVATION_PATH_KEY = "derivation_path"
    const val TRANSACTION_ID_KEY = "transaction_id"
    const val PROMO_CODE_KEY = "promo_code"
    const val REF_KEY = "ref"
    const val CAMPAIGN_KEY = "campaign"
    const val CAMPAIGN_ID_KEY = "campaignId"
    const val NAME_KEY = "name"
    const val ORDER_KEY = "order"
    const val INTERVAL_KEY = "interval"
    const val SECTION_KEY = "section"
    const val SCREEN_KEY = "screen"
    const val CATEGORY_ID_KEY = "category_id"
    const val NEWS_ID_KEY = "news_id"
    const val EARN_TYPE_KEY = "earn_type"
    const val FROM_TOKEN_ID_KEY = "from_token_id"
    const val FROM_NETWORK_ID_KEY = "from_network_id"
    const val TO_TOKEN_ID_KEY = "to_token_id"
    const val TO_NETWORK_ID_KEY = "to_network_id"
    const val FROM_AMOUNT_KEY = "from_amount"
    const val PROVIDER_ID_KEY = "provider_id"
    const val FROM_USER_WALLET_ID_KEY = "from_user_wallet_id"
    const val FROM_USER_ACCOUNT_ID_KEY = "from_user_account_id"
    const val TO_USER_WALLET_ID_KEY = "to_user_wallet_id"
    const val TO_USER_ACCOUNT_ID_KEY = "to_user_account_id"
    const val ACCOUNT_ID_KEY = "account_id"
    const val ENTRY_KEY = "entry"
}