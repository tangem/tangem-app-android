...
internal class MyPortfolioUMFactory(
    ...
    private fun Map<UserWallet, List<PortfolioData.CryptoCurrencyData>>.getAddButtonState(
    ...
    ): MyPortfolioUM.Tokens.AddButtonState {
        if (availableNetworks == null) return MyPortfolioUM.Tokens.AddButtonState.Loading
        ...
        return if (isAllAvailableNetworksAdded) {
    ...
    private fun List<PortfolioData.CryptoCurrencyData>.filterAvailableNetworks(
        ...
        return mapNotNull { cryptoCurrencyData ->
            cryptoCurrencyData.takeIf { networkIds.contains(it.status.currency.network.backendId) }
        }
    }
}