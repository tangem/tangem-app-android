...
internal class StakingViewModel @Inject constructor(
        ...
        if (isAssentState()) {
            viewModelScope.launch {
                ...
                val actionType = when (value.routeType) {
                    RouteType.STAKING -> StakingActionCommonType.ENTER
                    RouteType.UNSTAKING -> StakingActionCommonType.EXIT
                    RouteType.CLAIMING -> TODO()
                }
    ...
    override fun openRewardsValidators() {
        stateController.update { it.copy(routeType = RouteType.CLAIMING) }
        onNextClick()
    }
    ...
    override fun onActiveStake(activeStake: BalanceState) {
        stateController.update { it.copy(routeType = RouteType.UNSTAKING) }
