...
internal class WalletTokensListUMTransformer(
    ...
    private val apyConverter = ApyConverter(
    ...
    private fun toAccountRow(accountStatus: AccountStatus.CryptoPortfolio, isExpanded: Boolean): TangemTokenRowUM {
        ...
        val (topEndContent, bottomEndContent) = when (val accountBalance =
            accountStatus.tokenList.totalFiatBalance) {
            TotalFiatBalance.Failed -> {
                TangemTokenRowUM.EndContentUM.Content(
                    text = stringReference(StringsSigns.DASH_SIGN),
                ) to TangemTokenRowUM.EndContentUM.Content(
                    text = resourceReference(R.string.common_unreachable),
                )
            }
            ...
            TotalFiatBalance.Loading -> {
                TangemTokenRowUM.EndContentUM.Loading to TangemTokenRowUM.EndContentUM.Loading
            }
        }
    ...
    private fun toCurrencyRow(currencyStatus: CryptoCurrencyStatus, shouldShowPromo: Boolean): TangemTokenRowUM {
        val earnApyInfo = apyConverter.convert(currencyStatus)
    ...
    private fun toCurrencyRowTitle(
        ...
        earnApyInfo: ApyConverter.EarnApyInfo?,
    ...
    private fun isSingleCurrencyWalletWithToken(): Boolean {
        return selectedWallet is UserWallet.Cold && selectedWallet.scanResponse.cardTypesResolver.isSingleWalletWithToken()
    }
}