...
class DefaultUserWalletImageFetcher @Inject constructor(
    ...
    private val smallCache = MutableStateFlow(mutableMapOf<String, ArtworkUM>())
    private val largeCache = MutableStateFlow(mutableMapOf<String, ArtworkUM>())
    ...
    private fun save(cardId: String, size: ArtworkSize, artwork: ArtworkUM) {
        when (size) {
            ArtworkSize.LARGE -> largeCache.value[cardId] = artwork
            ArtworkSize.SMALL -> smallCache.value[cardId] = artwork
        }
    }
}