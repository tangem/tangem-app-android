...
internal class SwapInteractorImpl @Inject constructor(
    ...
    private suspend fun manageDex(
        ...
        // `txType=send` on the quote is the per-quote signal to switch to the CEX-style path.
    ...
    private suspend fun manageDexSolana(
        ...
        // Same per-quote re-route as in manageDex (see above).
        if (maybeQuotes.getOrNull()?.txType == ExpressTxType.SEND) {
            return manageCex(
                fromSwapCurrencyStatus = fromSwapCurrencyStatus,
                toSwapCurrencyStatus = toSwapCurrencyStatus,
    ...
    override suspend fun onSwap(
        ...
        // For DEX/DEX_BRIDGE the actual shape is decided by `swapData.transaction`: a DEX
        // transaction stays on the DEX path, a CEX transaction or null routes to the CEX path
        // (null means the quote already re-routed and didn't pre-build swapData).
        val resolvedFlow = when (swapProvider.type) {
