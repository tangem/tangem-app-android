...
internal class DefaultPortfolioSelectorController @Inject constructor(
    ...
    private var _selectedAccountSync: AccountId? = null
    ...
    override val selectedAccountSync: AccountId? get() = _selectedAccountSync

    override var isEnabled: MutableStateFlow<(UserWallet, AccountStatus) -> Boolean> = MutableStateFlow { _, _ -> true }

    init {
        _selectedAccount.tryEmit(_selectedAccountSync)
    }

    override fun selectAccount(accountId: AccountId?) {
        _selectedAccountSync = accountId
