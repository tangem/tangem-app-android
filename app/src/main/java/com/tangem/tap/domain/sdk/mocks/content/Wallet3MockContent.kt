...
/**
 * Wallet 3 (firmware v8.58+), personalized per the "Tangem 3.0" issuer config: batch `DD000000`,
 * `walletsCount = 10`, `securityDelay = 5000`, `allowBackup`/`allowHDWallets` = true. Backup is modeled
 * as active with 3 linked cards (4-card backup total) — the v8 SDK protocol extends backup linking up
 * to 4 cards (`Helper_TC08.2_BackupHashFourCards`), vs. Wallet 2's 2-card default.
 */
object Wallet3MockContent : MockContent {

    private val primaryCard = PrimaryCard(
        cardId = "DD00000000000107",
        batchId = "DD000000",
        ...
        manufacturer = Card.Manufacturer(
            name = "TANGEM",
            manufactureDate = Date(1749081600000), // 2026-06-05, per cardData.date
    ...
    override val cardDto = CardDTO(
        ...
        settings = CardDTO.Settings(
            securityDelay = 5000,
            maxWalletsCount = 10,
            isSettingAccessCodeAllowed = false, // config: allowSwapPIN = false
            isSettingPasscodeAllowed = false, // config: allowSwapPIN = false
            isResettingUserCodesAllowed = true,
            isLinkedTerminalEnabled = true,
            isBackupAllowed = true, // config: allowBackup = true
            supportedEncryptionModes = listOf(EncryptionMode.Strong, EncryptionMode.Fast, EncryptionMode.None),
            isFilesAllowed = true, // config: disableFiles = false
            isHDWalletAllowed = true, // config: allowHDWallets = true
            isKeysImportAllowed = true,
        ),
        ...
        // v8 protocol extends backup linking up to 4 cards total (primary + 3 backup), vs. Wallet 2's 2.
        backupStatus = CardDTO.BackupStatus.Active(3),
    )

    override val scanResponse = ScanResponse(
        card = cardDto,
        // No dedicated ProductType case exists for FW8 cards; production classification
        // (ScanProductTask.getWalletProductType) already resolves firmware >= 6.33 + isKeysImportAllowed
        // to Wallet2, so this mirrors real runtime behavior.
    ...
    override val successResponse = SuccessResponse(cardId = "DD00000000000107")
