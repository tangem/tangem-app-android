        ...
        { action ->
            when (action) {
                is GlobalAction.ScanFailsCounter.ChooseBehavior -> {
                    when (action.result) {
                        is CompletionResult.Success -> store.dispatch(
                            GlobalAction.ScanFailsCounter.Reset,
                        )
                ...
                is GlobalAction.UpdateWalletSignedHashes -> {
                    store.dispatch(
                        WalletAction.Warnings.CheckRemainingSignatures(action.remainingSignatures),
                    )
                }
                    ...
                    ifNotNull(
                        ...
                        scope.launch {
                            ...
                            store.dispatchOnMain(
                                GlobalAction.InitCurrencyExchangeManager.Success(exchangeManager),
                            )
                        }
                    }
                }
