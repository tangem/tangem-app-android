...
class AppCurrencyMiddleware(
            ...
            runCatching { walletRepository.getCurrencyList() }
                .onSuccess { response ->
                    val currenciesList = response.currencies
                        .map { with(it) { CurrencyDM(id, code, name, rateBTC, unit, type) } }
    ...
    private fun selectCurrency(action: WalletAction.AppCurrencyAction.SelectAppCurrency) {
        ...
        fiatCurrenciesPrefStorage.saveAppCurrency(with(action.fiatCurrency) { FiatCurrencyDM(code, name, symbol) })
    ...
    private fun List<CurrencyDM>.mapToUiModel(): List<FiatCurrency> {
        return this.map {
            FiatCurrency(
                code = it.code,
                name = it.name,
