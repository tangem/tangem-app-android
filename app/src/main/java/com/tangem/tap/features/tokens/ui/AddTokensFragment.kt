...
class AddTokensFragment : Fragment(R.layout.fragment_add_tokens),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) = with(binding) {
        ...
        store.state.globalState.analyticsHandlers?.triggerEvent(event = AnalyticsEvent.MANAGE_TOKENS_TAPPED)

        (activity as? AppCompatActivity)?.setSupportActionBar(toolbar)
        toolbar.setNavigationOnClickListener { activity?.onBackPressed() }

        val onSaveChanges = { tokens: List<TokenWithBlockchain>, blockchains: List<Blockchain> ->
            store.state.globalState.analyticsHandlers?.triggerEvent(event = AnalyticsEvent.TOKEN_LIST_SAVE)
            store.state.globalState.analyticsHandlers?.triggerEvent(event = AnalyticsEvent.TOKEN_TAPPED)
