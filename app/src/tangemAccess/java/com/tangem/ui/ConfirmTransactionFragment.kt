...
class ConfirmTransactionFragment :
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        amount = CoinEngine.Amount(
            arguments?.getString(Constant.EXTRA_AMOUNT) ?: "0",
                ...
                override fun onTextChanged(s: CharSequence, start: Int, before: Int, count: Int) {
                    try {
                        ...
                        if (!ctx.coinData!!.amountEquivalentDescriptionAvailable) {
                            tvFeeEquivalent.error = getString(
                                R.string.confirm_transaction_error_service_unavailable,
                            )
                        ...
                        if (sp.getBoolean(getString(R.string.pref_manual_editing_fee), false)) {
                            (activity as MainActivity).toastHelper
                                .showSingleToast(
                                    context,
                                    getString(R.string.confirm_transaction_warning_risk_delaying),
                                )
                        }
                    } catch (e: Exception) {
                        e.printStackTrace()
            ...
            if (UtilHelper.isOnline(requireContext())) {
                ...
                if (dtVerified == null || dtVerified!!.before(calendar.time)) {
                    finishWithError(
                        ...
                        getString(R.string.confirm_transaction_error_data_is_outdated),
                    )
                ...
                if (engineCoin!!.isNeedCheckNode && !nodeCheck) {
                    Toast.makeText(
                        context,
                        getString(R.string.confirm_transaction_error_cannot_reach_node),
                        Toast.LENGTH_LONG,
                    ).show()
                    return@setOnClickListener
                }

                val txFee = engineCoin.convertToAmount(
                    etFee.text.toString(),
                    tvCurrency2.text.toString(),
                )
                val txAmount = engineCoin.convertToAmount(
                ...
                if (!engineCoin.hasBalanceInfo()) {
                    finishWithError(
                        Activity.RESULT_CANCELED,
                        getString(R.string.confirm_transaction_error_cannot_check_balance),
                    )
                    return@setOnClickListener
                } else if (!engineCoin.isBalanceNotZero) {
                    finishWithError(
                        Activity.RESULT_CANCELED,
                        getString(R.string.general_wallet_empty),
                    )
                    return@setOnClickListener
                    ...
                    finishWithError(
                        Activity.RESULT_CANCELED,
                        getString(
                            R.string.confirm_transaction_error_incoming_transaction_unconfirmed,
                        ),
                    )
                    return@setOnClickListener
                }
                ...
                if (!engineCoin.checkNewTransactionAmountAndFee(txAmount, txFee, isIncludeFee)) {
                    finishWithError(
                        Activity.RESULT_CANCELED,
                        getString(R.string.prepare_transaction_error_not_enough_funds),
                    )
                    return@setOnClickListener
                }
                ...
                navigateForResult(
                    Constant.REQUEST_CODE_REQUEST_PIN2_,
                ...
                Toast.makeText(
                    context,
                    getString(R.string.general_error_no_connection),
                    Toast.LENGTH_SHORT,
                ).show()
            }
        }
    ...
    override fun onNavigationResult(requestCode: String, resultCode: Int, data: Bundle?) {
        ...
        if (requestCode == Constant.REQUEST_CODE_SIGN_TRANSACTION) {
            if (data != null) {
                if (data.containsKey(EXTRA_TANGEM_CARD_UID) &&
                    data.containsKey(EXTRA_TANGEM_CARD)
                ) {
            ...
            if (resultCode == Constant.RESULT_INVALID_PIN_ && requestPIN2Count < 2) {
                ...
                bundle.putString(
                    Constant.EXTRA_MODE,
                    PinRequestFragment.Mode.RequestPIN2.toString(),
                )
            ...
            if (resultCode == Activity.RESULT_OK) {
                ...
                Toast.makeText(
                    context,
                    R.string.confirm_transaction_error_pin_2_is_required,
                    Toast.LENGTH_LONG,
                ).show()
            }
        }
