...
abstract class ChainProcessor<E, R> {
    private val chains: MutableList<Chain<E, R>> = mutableListOf()

    protected fun addChains(vararg chains: Chain<E, R>) {
        this.chains.addAll(chains)
    }

    protected suspend fun launchChains(
        initial: R,
        returnOnFirstError: Boolean = true,
    ): Either<E, R> {
        return launchChains(initial = Either.Right(initial), returnOnFirstError)
    }

    private suspend fun launchChains(
        initial: Either<E, R>,
        returnOnFirstError: Boolean = true,
    ): Either<E, R> {
        var result = initial
