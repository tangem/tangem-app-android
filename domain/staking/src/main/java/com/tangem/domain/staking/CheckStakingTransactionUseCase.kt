 ...
 * (Ethereum/Binance/Solana/Tron/Cosmos/Cardano); not for toggle-off, pass-through networks
 * (e.g. Polygon), or null payload.
 *
 * Additionally reports a [SuspiciousStakingTransactionException] non-fatal to Firebase whenever the
 * resulting verdict is not SAFE (WARNING or UNSAFE), so flagged transactions are observable in
 * Crashlytics.
 */
class CheckStakingTransactionUseCase(
    ...
    suspend operator fun invoke(
        ...
        val verdict = when (network) {
            ...
            // Polygon is intentionally excluded from verification (pass-through SAFE): see `else` below.
            NetworkType.BINANCE,
            NetworkType.SOLANA,
    ...
    private fun reportSuspiciousTransaction(
        verdict: StakingTransactionVerdict,
        token: String,
        blockchain: String,
        provider: String,
    ) {
        if (verdict == StakingTransactionVerdict.SAFE) return

        analyticsExceptionHandler.sendException(
            ExceptionAnalyticsEvent(
                exception = SuspiciousStakingTransactionException(verdict),
                params = mapOf(
