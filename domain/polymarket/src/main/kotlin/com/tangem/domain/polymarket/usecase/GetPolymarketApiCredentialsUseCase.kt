...
/**
 * Reads the stored CLOB credentials for [ownerAddress]. Local only: it never reaches the network and never
...
class GetPolymarketApiCredentialsUseCase(
    ...
    suspend operator fun invoke(ownerAddress: String): PolymarketApiCredentials? =
        credentialsStore.get(ownerAddress = ownerAddress)
}