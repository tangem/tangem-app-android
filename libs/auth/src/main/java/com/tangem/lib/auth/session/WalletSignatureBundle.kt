...
data class WalletSignatureBundle(
    ...
    // ByteArray uses referential equality in the data-class-generated equals/hashCode; override with
    // content comparison so two bundles with identical bytes compare equal.
    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other !is WalletSignatureBundle) return false

        return walletSignature.contentEquals(other.walletSignature) &&
            walletSignatureSalt.contentEquals(other.walletSignatureSalt) &&
            cardSignature.contentEquals(other.cardSignature) &&
            cardSignatureSalt.contentEquals(other.cardSignatureSalt) &&
            walletStatusByte == other.walletStatusByte
    }

    override fun hashCode(): Int {
        var result = walletSignature.contentHashCode()
        result = 31 * result + walletSignatureSalt.contentHashCode()
        result = 31 * result + (cardSignature?.contentHashCode() ?: 0)
        result = 31 * result + (cardSignatureSalt?.contentHashCode() ?: 0)
        result = 31 * result + (walletStatusByte?.hashCode() ?: 0)
        return result
    }
}
