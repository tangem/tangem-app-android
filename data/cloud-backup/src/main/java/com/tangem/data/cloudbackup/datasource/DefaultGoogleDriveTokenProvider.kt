 ...
 * 3. bails out with [CloudBackupError.NetworkError] when the device is offline;
 * 4. on a cache miss, delegates to [GoogleDriveAuthorizer] (honoring `interactive`) and caches the result.
 *
...
internal class DefaultGoogleDriveTokenProvider(
    ...
    override suspend fun getAccessToken(interactive: Boolean): Either<CloudBackupError, String> {
        ...
        return authMutex.withLock {
            ...
            if (!networkConnectionManager.isOnline) return@withLock CloudBackupError.NetworkError.left()
